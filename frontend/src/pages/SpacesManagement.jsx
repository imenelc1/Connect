...
export default function SpacesPage() {
  ...
  useEffect(() => {
    const fetchSpaces = async () => {
      try {
        ...
        const res = await fetch("${import.meta.env.VITE_API_BASE}/api/spaces/", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editIndex) {
        const res = await fetch(`${import.meta.env.VITE_API_BASE}/api/spaces/admin/${editIndex}/update/`, {
          method: "PUT",
        ...
        const res = await fetch("${import.meta.env.VITE_API_BASE}/api/spaces/admin/create/", {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify(payload),
        });
  ...
  const handleDeleteSpace = async (id_space) => {
    ...
    try {
      const token = localStorage.getItem("admin_token");
      const res = await fetch(`${import.meta.env.VITE_API_BASE}/api/spaces/space/${id_space}/delete/`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleOpenSpaceStudents = async (space) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE}/api/spaces/space/${space.id}/details/`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      const resAll = await fetch(
        "${import.meta.env.VITE_API_BASE}/api/users/students-with-progress/",
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleAddStudentsToSpace = async (e) => {
    ...
    try {
      ...
      for (let studentId of selectedStudents) {
        ...
        const res = await fetch("${import.meta.env.VITE_API_BASE}/api/spaces/admin/add-students/", {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
  ...
  const handleRemoveStudentFromSpace = async (student) => {
    ...
    try {
      ...
      const res = await fetch(
        "${import.meta.env.VITE_API_BASE}/api/spaces/admin/remove-student/",
        {
          method: "POST",
          headers: {
