...
export default function StudentSignUp() {
  ...
  // État pour stocker les erreurs de chaque champ
  ...
  const validateForm = () => {
    ...
    // Validation du contenu (pas de chiffres dans nom/prénom)
    ...
    // Email obligatoire et format correct
    if (!formData.email) newErrors.email = t("emailRequired");
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email))
      newErrors.email = t("invalidEmail");
    // Mot de passe obligatoire et longueur minimale
  ...
  const handleSubmit = async e => {
    ...
    // Préparation du payload pour l'AP
    const payload = {
  ...
  return (
    ...
    {/* Onglets Auth (signIn / signUp) */}
      ...
      />
      {/* Conteneur principal : formulaire + mascotte */}
      <div className="flex flex-col lg:flex-row w-full max-w-[1000px] min-h-[650px] bg-card rounded-3xl shadow-lg overflow-hidden relative mt-2">
        ...
        <div className="w-full md:w-1/2 p-10">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            {/* Inputs mot de passe et confirmation avec toggle visibilité */}
            <Input
              label={t("password")}
              name="password"
            ...
            />
            {/* Date de naissance et matricule */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
