...
export default function QuizPreview() {
  ...
  return (
    <div className="min-h-screen flex flex-col px-4 md:px-8 py-4 md:py-6"
         style={{ background: "rgb(var(--color-bg))" }}>

      {/* Logo */}
      <div className="fixed -top-11 left-2 z-50 w-20 md:w-28">
        <Logo />
      </div>
      ...
      <div className="absolute top-4 right-4 flex gap-2">
        ...
        <button
          ...
          className="px-2 md:px-3 py-1 rounded bg-gray-200 text-xs md:text-sm"
        >
      ...
      {/* Title */}
      <h1 className="text-xl md:text-3xl font-bold text-center mt-10 mb-4 md:mb-6"
          style={{ color: "rgb(var(--color-primary))" }}>
        {quiz.title || t("title")}
      </h1>

      {/* Description */}
      <p className="text-sm text-gray-600 mb-4 md:mb-6 px-2 text-left">
        {quiz.description || t("description")}
      </p>

      {/* Overview section → mobile stacked, desktop spaced */}
      <div className="flex-1 w-full rounded-2xl shadow-lg mt-4 md:mt-10 p-4 md:p-6"
           style={{ background: "rgb(var(--color-bg))" }}>

        <div className="flex flex-col md:flex-row md:justify-center gap-3 md:gap-x-80 text-xs md:text-sm mb-6">

          <div className="px-6 md:px-8 py-2 rounded-md shadow-sm flex items-center gap-2 justify-center"
               style={{ background: "rgb(var(--color-primary))", color: "white" }}>
            <FaClock /> {quiz.duration} {t("minutes")}
          </div>

          <div className="px-6 md:px-8 py-2 rounded-md shadow-sm flex items-center gap-2 justify-center"
               style={{ background: "rgb(var(--color-tertiary))", color: "white" }}>
        ...
        <div className="flex flex-col gap-4">
          {quiz.questions.map((q, index) => (
            <div key={index}
                 className="rounded-xl shadow-sm p-3 md:p-4"
                 style={{ background: "var(--grad-5)" }}>

              <h3 className="font-semibold mb-1 flex items-center gap-2 text-sm md:text-base">
                <div className="w-5 h-5 md:w-6 md:h-6 flex items-center justify-center rounded-md text-white text-xs md:text-sm font-bold"
                     style={{ background: "rgb(var(--color-primary))" }}>
                  {index + 1}
                </div>
              ...
              <div className="flex flex-col gap-2">
                {q.answers.map((a, i) => (
                  <label key={i}
                    ...
                    />
                    <span className="text-sm md:text-base">{a.text || t("answerOption")}</span>
                  </label>
                ))}
              </div>
