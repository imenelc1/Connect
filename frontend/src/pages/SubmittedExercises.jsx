  ...
  `}
      >
        <h1 className="text-3xl font-semibold text-muted">
          {t("submittedExercises") || "Exercices Soumis"}
        </h1>
        <div className="text-muted font-medium text-sm border border-primary px-3 py-1 rounded-md shadow-sm bg-surface inline-block mt-2">
          {t("total") || "Total"}: {filtered.length}{" "}
          {t("exercises") || "exercices"}
        </div>
        ...
        <div className="flex flex-col gap-5 mt-6">
          {loading ? (
            ...
            <div className="text-center py-10 text-gray-500">
              {t("noSubmittedExercises")}{" "}
              {filter !== "All" ? filter.toLowerCase() : ""}{" "}
              {t("exercisesFound") || "exercices trouvés."}
            </div>
          ) : (
            filtered.map((ex) => {
              ...
              className={`absolute top-3 right-3 flex items-center gap-1 text-xs px-2 py-1 rounded 
                ${
                  ex.status === "Soumis"
                    ...
                    : ex.status === "Brouillon"
                      ? "bg-pink/30"
                      : "bg-gray-300/30" // fallback neutre
                }`}
            >
              {ex.status === "Soumis" ? <CheckCircle size={14} /> : <Clock size={14} />}
              {ex.status === "Soumis"
                ? t("submitted") // Soumis → Submitted
                : ex.status === "Brouillon"
                  ? t("brouillon")     // Brouillon → Draft
                  : ex.status}   
            </span>
                  ...
                  {/* Détails */}
                  <div
                    className={`${isExpanded ? "block" : "hidden"} sm:block mt-4 space-y-3`}
                  >
                    {ex.hasFeedback && (
                      <div className="flex flex-col">
                        ...
                        <div className="p-3 rounded-md bg-grad-3 border border-primary/20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-10">
          <div className="flex items-center justify-between bg-grad-3 border border-secondary2 p-5 rounded-xl">
            <div className="flex items-center gap-3">
              ...
              <span className="font-regular text-md text-muted">
                {t("submitted") || "Soumis"}
              </span>
            </div>
          ...
          <div className="flex items-center justify-between bg-grad-4 border border-pink p-5 rounded-xl">
            <div className="flex items-center gap-3">
              ...
              <span className="font-regular text-md text-pink">
                {t("brouillon") || "Brouillon"}
              </span>
            </div>
