...
export default function StudentsManagement() {
        ...
        const studentsWithRealProgress = await Promise.all(
          studentsData.map(async (s) => {
            const progRes = await fetch(
              `https://connect-1-t976.onrender.com/api/dashboard/global-progress/${s.id}/`,
              { headers: { Authorization: `Bearer ${token}` } }
            );
  ...
  const handleDelete = async (studentId) => {
    ...
    try {
      const res = await fetch(`https://connect-1-t976.onrender.com/api/users/admin/users/${studentId}/`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await fetch(`https://connect-1-t976.onrender.com/api/users/etudiants/${editStudent.id}/update/`, {
        method: "PUT",
  ...
  const handleCreateStudent = async (e) => {
    ...
    try {
      const res = await fetch("https://connect-1-t976.onrender.com/api/users/admin/etudiants/create/", {
        method: "POST",
        headers: {
