...
export default function Navbar() {
  ...
  return (
    <aside
      className={`h-screen bg-background rounded-3xl shadow-2xl p-3 flex flex-col justify-between
    ...
    >
      ...
      {/* HEADER */}
      <div className="flex items-center justify-center p-2 bg-background rounded-2xl shadow-sm -mt-1 mb-1">
        <IconeLogoComponent
      ...
      <nav className="mt-2 flex flex-col gap-1 font-medium pb-3 border-b border-surface/60">
        {links.map((item, i) => (
          <NavLink
            ...
            to={item.href}
            className={({ isActive }) =>
              `flex items-center px-3 py-2 rounded-xl border text-sm transition-all ${
                isActive
