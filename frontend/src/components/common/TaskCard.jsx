...
export default function TaskCard({ title, date, etat, code, feedback, exerciceId, tentativeId }) {
  ...
  try {
    ...
    await axios.post(
      `${BACKEND_URL}/api/feedback-exercice/`,
      {
        contenu: newFeedback,
        exercice: exerciceId,
        tentative: tentativeId,
      },
      { headers: { Authorization: `Bearer ${token}` } }
    );

    setCurrentFeedback(newFeedback); // ← update local
    setNewFeedback("");
...
<div className="mt-4">
  {currentFeedback ? (
    ...
    <div className="flex flex-col gap-2">
      ...
      />
      <button
        className="flex px-4 py-2 bg-purple text-white rounded-xl self-start items-center gap-2 mt-2"
